...
export function TemplateEditor({
  ...
  const [fields, setFields] = useState<TextField[]>(() => {
    // FIX: Restore isDefault flag when loading saved fields
    return initialFields.map(field => ({
      ...field,
      isDefault: field.isDefault !== undefined 
        ? field.isDefault 
        : DEFAULT_FIELD_NAMES.includes(field.name) // Check by name if flag is missing
    }));
  });
  ...
  const addDefaultField = () => {
    ...
    const newField: TextField = {
      ...
      bold: false,
      italic: false,
      isDefault: true, // CRITICAL: Set this flag
  ...
  const addCustomField = () => {
    ...
    const newField: TextField = {
      ...
      bold: false,
      italic: false,
      isDefault: false, // CRITICAL: Explicitly set to false
      fieldType: "text",
  ...
  const handleSave = async () => {
    ...
    // FIX: Ensure isDefault flag is preserved when saving
