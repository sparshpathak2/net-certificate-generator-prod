...
interface TemplateEditorProps {
  ...
  isSaving?: boolean;
  isSaveSuccess?: boolean; // ✅ Add this to track save success
}
...
export function TemplateEditor({
  ...
  // ✅ Update savedFields when initialFields changes (after successful save)
  useEffect(() => {
    setFields(initialFields);
    setSavedFields(initialFields);
  }, [initialFields]);

  // ✅ Also update savedFields when save is successful
  useEffect(() => {
    if (isSaveSuccess) {
      setSavedFields([...fields]);
    }
  }, [isSaveSuccess, fields]);
...
const handleGenerateClick = () => {
  ...
  // Show loading toast while generating
      ...
      <div className="w-96 flex flex-col border-l bg-white">
        ...
        <div className="flex flex-col gap-2 p-4 border-t bg-white">
          ...
          <Button
            ...
            className="w-full gap-2"
            disabled={!hasAnySavedFields || hasUnsavedChanges || isSaving}
          >
            <Save size={16} />
