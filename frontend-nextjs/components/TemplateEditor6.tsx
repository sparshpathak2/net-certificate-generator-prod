...
const lastSavedFieldsRef = useRef<TextField[]>(initialFields);
...
useEffect(() => {
  if (isSaveSuccess && !isLocalSaving) {
    console.log("Save success detected, updating savedFields from:", savedFields.length, "to:", initialFields.length);
    if (JSON.stringify(initialFields) !== JSON.stringify(savedFields)) {
      setSavedFields([...initialFields]);
      lastSavedFieldsRef.current = [...initialFields];
    }
  }
}, [isSaveSuccess, initialFields, isLocalSaving]);
...
useEffect(() => {
  console.log("State changed - fields:", fields.length, "savedFields:", savedFields.length, "hasUnsavedChanges:", hasUnsavedChanges);
}, [fields, savedFields]);
...
//   };
  ...
  try {
    await onSave(fields);
    // After successful save, update savedFields immediately
    console.log("Save completed, updating savedFields to:", fields.length);
    setSavedFields([...fields]);
    lastSavedFieldsRef.current = [...fields];
    toast.success("Template saved successfully!");
  } catch (error) {
    toast.error("Failed to save template");
  } finally {
