...
interface TextField {
  ...
  pdfWidth?: number;
  pdfHeight?: number;
  placeholder?: string; // Add placeholder to store original default text
}
...
// Default field templates - used for identification
...
const DEFAULT_FIELDS = [
  {
    ...
    isRequired: true,
    fieldType: "text",
    mappingKey: "recipientName", // Key for Excel mapping
  },
  {
    name: "recipientEmail",
...
export function TemplateEditor({
  ...
  const [fields, setFields] = useState<TextField[]>(() => {
    // FIX: Restore isDefault flag and placeholder when loading saved fields
    return initialFields.map(field => {
      const isDefaultField = field.isDefault !== undefined 
        ? field.isDefault 
        : DEFAULT_FIELD_NAMES.includes(field.name);
      
      // Find matching default field to get placeholder
      const defaultField = DEFAULT_FIELDS.find(f => f.name === field.name);
      
      return {
        ...field,
        isDefault: isDefaultField,
        placeholder: field.placeholder || defaultField?.placeholder || field.text,
      };
    });
  });
  
  const [lastSavedFields, setLastSavedFields] = useState<TextField[]>(() => {
    return initialFields.map(field => {
      const isDefaultField = field.isDefault !== undefined 
        ? field.isDefault 
        : DEFAULT_FIELD_NAMES.includes(field.name);
  ...
  useEffect(() => {
    if (!isInitialized.current && initialFields.length > 0) {
      // FIX: Ensure isDefault is properly set on load
      const fixedFields = initialFields.map(field => {
        const isDefaultField = field.isDefault !== undefined 
          ? field.isDefault 
          : DEFAULT_FIELD_NAMES.includes(field.name);
      ...
      setLastSavedFields(fixedFields);
  ...
  const addDefaultField = () => {
    ...
    const newField: TextField = {
      ...
      placeholder: defaultField.placeholder, // Store placeholder
      x: cx,
      y: cy,
      fontSize: 24,
...
//     // FIX: Ensure isDefault flag and placeholder are preserved when saving
//     const fieldsToSave = fields.map((field) => ({
//       ...field,
//       isDefault: field.isDefault !== undefined 
//         ? field.isDefault 
//         : DEFAULT_FIELD_NAMES.includes(field.name),
//       // Don't save the current text value for default fields if it's been changed
//       // Instead, save the placeholder as the default text
//       text: field.isDefault ? field.placeholder || field.text : field.text,
//       pdfWidth: pdfPointDimensions.width,
//       pdfHeight: pdfPointDimensions.height,
//     }));
...
//   };
  ...
  const handleSave = async () => {
    ...
    // ✅ Spread the entire field and only override what's necessary
    const fieldsToSave = fields.map((field, index) => ({
        ...field,  // Keep all existing properties
        isDefault: field.isDefault !== undefined 
            ? field.isDefault 
            : DEFAULT_FIELD_NAMES.includes(field.name),
        text: field.isDefault ? (field.placeholder || field.text) : field.text,
  ...
  // Function to get field mapping key for Excel
  const getFieldMappingKey = (field: TextField): string => {
    if (field.isDefault) {
      const defaultField = DEFAULT_FIELDS.find(f => f.name === field.name);
      return defaultField?.mappingKey || field.name;
    }
    return field.name;
  };
  ...
  const customFieldsInTemplate = fields.filter(f => f.isDefault !== true);

  // Prepare fields for MapData component with proper mapping keys
  const fieldsForMapping = fields.map(field => ({
    ...field,
    mappingKey: getFieldMappingKey(field),
      ...
      <div className="w-96 flex flex-col border-l bg-white">
                      ...
                      <CardContent className="p-3 pt-1">
                        <div className="text-xs text-gray-500 space-y-0.5">
                          ...
                          <div className="text-blue-600 mt-1">
                            Mapping Key: {getFieldMappingKey(field)}
                          </div>
                        </div>
                      </CardContent>
                    </div>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {selectedField.isDefault && (
                    <p className="text-xs text-gray-400">
                      This text will be replaced by Excel data during certificate generation
                    </p>
                  )}
                </div>
        ...
        <div className="flex flex-col gap-2 p-4 border-t bg-white">
          ...
          <MapData 
            ...
            fields={fieldsForMapping}  // Pass enhanced fields with mapping keys
          >
            <Button
              className="w-full gap-2"
