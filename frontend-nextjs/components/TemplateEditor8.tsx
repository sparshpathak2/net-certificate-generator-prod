...
export function TemplateEditor({
  ...
  // ✅ Initialize state from initialFields only once on mount
  ...
  // ✅ Track if we've initialized once
  ...
  // ✅ Only initialize on first mount, don't sync on every change
  useEffect(() => {
...
//     console.log(`Actual PDF dimensions from react-pdf: ${width} x ${height}`);
//   };
...
const onPageLoadSuccess = (page: any) => {
    ...
    console.log('Page originalWidth:', page.originalWidth);
    console.log('Page originalHeight:', page.originalHeight);
    ...
    // Calculate scale factor between viewport and original
    const scaleX = viewport.width / page.originalWidth;
    const scaleY = viewport.height / page.originalHeight;
  ...
  // Get canvas coordinates for dragging
...
//   };
  ...
  const getCanvasCoordinates = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    console.log(`Display click: (${e.clientX - rect.left}, ${e.clientY - rect.top})`);
    console.log(`Display size: ${displayWidth} x ${displayHeight}`);
    console.log(`PDF size: ${pdfWidth} x ${pdfHeight}`);
    console.log(`Scale: ${scaleX}, ${scaleY}`);
    console.log(`PDF coordinates: (${x}, ${y})`);
    
    return { x, y };
};

  // Handle click to select field (for existing fields)
  ...
  // Handle double click to add new field at cursor position
  ...
  const handleSave = async () => {
    ...
    // Include the actual PDF dimensions when saving
    const fieldsWithDimensions = fields.map(field => ({
        ...field,
        pdfWidth: pageDimensions.width,
        pdfHeight: pageDimensions.height,
    }));
    ...
    try {
        await onSave(fieldsWithDimensions);
        setLastSavedFields(fieldsWithDimensions);
          ...
          <div className="p-4 space-y-4">
            ...
            {fields.length === 0 && (
              <div className="text-center text-gray-500 py-8">
                ...
                <p className="text-xs text-gray-400 mt-2">Double-click on template to add field at cursor position</p>
              </div>
            )}
