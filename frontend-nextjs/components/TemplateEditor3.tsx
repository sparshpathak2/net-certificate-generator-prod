...
export function TemplateEditor({
  ...
  const [hasSavedFields, setHasSavedFields] = useState(initialFields.length > 0);
  ...
  const [isFieldsSaving, setIsFieldsSaving] = useState(false);
  ...
  const onImageLoad = () => {
    if (imageRef.current) {
      ...
      // Draw image and fields on canvas
      drawImageWithFields();
    }
  };
  ...
  // Redraw when fields or selection changes
  useEffect(() => {
...
//   const handleSave = () => {
//   onSave?.(fields);
//   setHasSavedFields(true); // Mark as saved after successful save
// };

// Update savedFields when initialFields changes (e.g., after successful save from parent)
useEffect(() => {
  setSavedFields(initialFields);
}, [initialFields]);

const hasUnsavedChanges = JSON.stringify(fields) !== JSON.stringify(savedFields);
const hasAnySavedFields = savedFields.length > 0;

const handleSave = () => {
  onSave?.(fields); // Parent mutation handles the API call and isSaving state
};

// const handleSave = async () => {
//   setIsFieldsSaving(true);
//   try {
//     await onSave?.(fields);
//     // After successful save, check if there are any fields
//     setHasSavedFields(fields.length > 0); // ✅ Update based on actual fields count
//   } catch (error) {
//     console.error("Save failed:", error);
//   } finally {
//     setIsFieldsSaving(false);
//   }
// };

// // Track if there are any unsaved changes
// const hasUnsavedChanges = fields.length !== initialFields.length || 
      ...
      <div className="w-96 flex flex-col border-l bg-white">
        ...
        <div className="flex flex-col gap-2 p-4 border-t bg-white">
          ...
          <Button
            ...
            // disabled={!hasSavedFields || isSaving}
             disabled={!hasSavedFields || hasUnsavedChanges || isSaving}
          >
            <Save size={16} />
