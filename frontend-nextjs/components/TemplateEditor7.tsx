...
//   };
  ...
  // When user clicks on the PDF viewer
const handleCanvasClick = (e) => {
    // Get the actual PDF dimensions (not the displayed size)
    const { width: pdfWidth, height: pdfHeight } = pageDimensions;
    
    // Get the displayed container size
    ...
    // Convert click coordinates to actual PDF coordinates
    const actualX = (e.clientX - containerRect.left) * scaleX;
    const actualY = (e.clientY - containerRect.top) * scaleY;
    ...
    const newField = {
        ...
        x: actualX,
        y: actualY,
        pdfWidth: pdfWidth,   // Store the PDF dimensions
        pdfHeight: pdfHeight, // at the time of placement
    };
};
...
//     try {
//       await onSave(fields);
//       // ✅ Update lastSavedFields to match current fields after successful save
...
//   };
  ...
  const handleSave = async () => {
    ...
    try {
        ...
        // Also update lastSavedFields with dimensions
